...
for letter in top_soup.find(id='maincontent2').find('p').find_all('a'): # loop through every alphabet letter shown in the top nav link
	...
	charities_section = soup.find(id= 'maincontent2') # section on the page with the list of charities that start with current letter
	...
	for charity in charities_list:
		...
		charity_name = charity.contents[0] # alternately you could use charity.text to get charity name
